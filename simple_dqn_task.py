...
# config_file_path = "rocket_basic.cfg"
# config_file_path = "basic.cfg"

# Converts and downsamples the input image
def preprocess(img):
    img = img[0] # skimage.color.rgb2gray(img)
...
def perform_learning_step(epoch):
    ...
    # 1) Obtain the state (image_buffer) of the game object and preprocess it

    # 2) Select an action: 
        # 2.1) use eps-greedy policy (hint: use get_best_actio())
        # 2.2) decay the exploration rate (eps) with inreasing epoch number

    # 3) Make the action (hint: repeat the same action for several ticks (frame_repeat)
    # With probability eps make a random action.
    game.make_action(choice(actions), frame_repeat)

    # 4) Get the next state and preprocess it
        # (hint: you need also check whether the episode is finished)

    # 5) Use learn_from_transition() to update the weights of the network
